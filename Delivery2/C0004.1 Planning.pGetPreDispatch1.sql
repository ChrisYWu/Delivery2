...
Begin
	...
	If (@Debug = 1)
	Begin
		Select 'Branch/MerchGroup type'
		Select * From Setup.MerchGroup 
					Where MerchGroupID = @MerchGroupID 
	...
	Begin
		...
		Select pd.DispatchDate, pd.SAPAccountNumber, 1, Min(PlannedArrival) PlannedArrival, Null As EstimatedArrivalTime, Null As ArrivalTime, Null ActualDeparture, 0, Count(*) Cnt
		...
		Group By pd.DispatchDate, pd.SAPAccountNumber
		
		If(@Debug = 1)
		Begin
			Select '--- Planned Stops ----'
	...
	Begin
		Insert Into @StoreDelivery
		Select pd.DispatchDate, pd.SAPAccountNumber, 0, PlannedArrival, Null EstimatedArrival, Null ActualArrival, ActualArrival ActualDeparture, 0, 0
		From Planning.PreDispatch pd
	...
	Begin
		...
		Select * From @StoreDelivery Order By SAPAccountNumber
	End
	--^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^--
...
--Where SAPAccountNumber = 11321261
