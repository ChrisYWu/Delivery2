...
/*
Change Log:
2019-02-11		Change made to support DSD Xform-Sales Schedule lock project.  Query for item 4 updated.
*/
...
AS
	...
	From OpenQuery(RM_QA, '
        ...
        UNION
        SELECT  RS.ROUTE_NUMBER,
        RS.CUSTOMER_NUMBER, 
        RS.LOCATION_ID, 
        case length(replace(SS.WEEKS_SERVICED, '' '', '''')) when 1 THEN ''M''when 2 THEN ''B'' when 4 THEN ''W'' else ''A'' END as FREQUENCY,
        SS.START_DATE, 
        DS.DELIVERY_ROUTE, 
        SS.SCHEDULE_MAP AS SEQUENCE_NUMBER,
        ''0'' AS SEASONAL , 
        TO_DATE(''02-APR-06'', ''DD-MON-YY'') AS SEASONAL_START_DATE, 
        TO_DATE(''31-DEC-99'', ''DD-MON-YY'') AS SEASONAL_END_DATE
        FROM ACEUSER.ROUTE_SCHEDULE RS
        JOIN ACEUSER.SALES_SCHEDULE SS on RS.CUSTOMER_NUMBER = SS.CUSTOMER_NUMBER
        LEFT JOIN ACEUSER.DELIVERY_SCHEDULE DS on RS.CUSTOMER_NUMBER = DS.CUSTOMER_NUMBER
        WHERE RS.LOCATION_ID IN (SELECT location_id FROM ACEUSER.SALES_SCHEDULE_LOCATION_CONFIG WHERE ACTIVE=1 and CONFIG_VALUE=1)
	')
